...
class PostService {
  public async addPostToDb(userId: string, postData: IPostDocument): Promise<void> {
  ...
  public async getPosts(
    ...
    if (filter.imgId && filter.gifUrl) {
      query = { $or: [{ imgId: { $ne: '' } }, { gifUrl: { $ne: '' } }] };
    } else {
      query = filter;
    }
