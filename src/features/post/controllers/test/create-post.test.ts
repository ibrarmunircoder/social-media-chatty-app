...
describe('Create', () => {
  ...
  describe('post', () => {
    it('should send correct json response', async () => {
      ...
      const postData = spy.mock.calls[0][0].postData;
      expect(postServer.socketIOPostObject.emit).toHaveBeenCalledWith('add post', postData);
      ...
      expect(postQueue.addPostJob).toHaveBeenCalledWith('addPostToDb', { key: req.currentUser?.userId, value: postData });
  ...
  describe('postWithImage', () => {
    it('should throw an error if image is not available', async () => {
      ...
      const req: Request = postMockRequest(newPost, authUserPayload) as Request;
      const res: Response = postMockResponse();
      await CreatePostController.createPostWithImage(req, res).catch((error: CustomError) => {
        expect(error.statusCode).toEqual(400);
        expect(error.serializeErrors()[0].message).toEqual('Image is a required field');
      });
    });
    it('should throw an upload error', async () => {
      ...
      await CreatePostController.createPostWithImage(req, res).catch((error: CustomError) => {
        expect(error.statusCode).toEqual(400);
        expect(error.serializeErrors()[0].message).toEqual('Upload error');
      });
    });
    it('should send correct json response', async () => {
      ...
      await CreatePostController.createPostWithImage(req, res);
      const postData = spy.mock.calls[0][0].postData;
      expect(postServer.socketIOPostObject.emit).toHaveBeenCalledWith('add post', postData);
      expect(PostCache.prototype.savePostToCache).toHaveBeenCalledWith({
        key: spy.mock.calls[0][0].key,
