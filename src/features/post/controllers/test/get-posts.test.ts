...
describe('Get', () => {
  ...
  describe('posts', () => {
    it('should send correct json response if posts exist in cache', async () => {
      const req = postMockRequest(newPost, authUserPayload, { page: '1' }) as Request<{ page: string }, unknown, unknown>;
      ...
      await GetPostController.getPosts(req, res);
  ...
  describe('postWithImages', () => {
    it('should send correct json response if posts exist in cache', async () => {
      ...
      await GetPostController.getPostsWithImages(req, res);
