...
export class GetUserProfileController {
  public static async all(req: Request, res: Response): Promise<void> {
    const { page } = req.params;
    const skip: number = (parseInt(page) - 1) * PAGE_SIZE;
    const limit: number = PAGE_SIZE * parseInt(page);
    const newSkip: number = skip === 0 ? skip : skip + 1;
    const allUsers = await GetUserProfileController.allUsers({
      newSkip,
      limit,
      skip,
      userId: `${req.currentUser!.userId}`
    });
    const followers: IFollowerData[] = await GetUserProfileController.followers(`${req.currentUser!.userId}`);
  ...
  public static async profileByUserId(req: Request, res: Response): Promise<void> {
  ...
  public static async profileAndPosts(req: Request, res: Response): Promise<void> {
  ...
  public static async randomUserSuggestions(req: Request, res: Response): Promise<void> {
  ...
  private static async allUsers({ newSkip, limit, skip, userId }: IUserAll): Promise<IAllUsers> {
    let users;
    let type = '';
    ...
    const totalUsers: number = await GetUserProfileController.usersCount(type);
    return { users, totalUsers };
  }

  private static async usersCount(type: string): Promise<number> {
    const totalUsers: number = type === 'redis' ? await userCache.getTotalUsersInCache() : await userService.getTotalUsersInDB();
    return totalUsers;
  }

  private static async followers(userId: string): Promise<IFollowerData[]> {
