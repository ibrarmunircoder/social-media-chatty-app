...
import { addImageSchema } from '@images/schemes/image';
...
export class AddImageController {
  @zodValidation(addImageSchema)
  public static async profileImage(req: Request, res: Response): Promise<void> {
  ...
  @zodValidation(addImageSchema)
  public static async backgroundImage(req: Request, res: Response): Promise<void> {
    const { version, publicId }: IBgUploadResponse = await AddImageController.backgroundUpload(req.body.image);
